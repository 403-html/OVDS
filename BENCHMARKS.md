...
| Date / state | 2026-06-17, GPU idle |
...
- CPU: `SigningKey::generate` + pubkey serialization on all Rayon threads for 5 s.
  ...
  rate.
- GPU: median of 7 rounds of 40 `keygen_dispatch` calls (warm-up excluded). Each
  dispatch generates `threads * BATCH_K` = 16384 * 64 = 1,048,576 candidates and
  screens them on-device.
